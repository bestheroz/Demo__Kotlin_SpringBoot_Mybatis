...
dependencies {
    ...
    // Spring
    // Boot 4 에서 spring-boot-starter-web 은 spring-boot-starter-webmvc 로 이름이 바뀌었다.
    implementation(libs.spring.boot.starter.webmvc)
    implementation(libs.spring.boot.starter.security)
    implementation(libs.aspectjweaver)

    // Database
    // mysql-connector-j·jakarta.persistence-api·hibernate-core 는 BOM 도 관리하지만 원래부터 버전을 명시했다.
    // BOM 보다 앞서 체험하려는 핀으로 보고 LATEST 로 올린다.
    implementation(libs.mysql.connector.j)
    implementation(libs.p6spy.spring.boot.starter)
    implementation(libs.mybatis.spring.boot.starter)
    // 제네릭 CRUD(MybatisRepository). 엔티티 필드 매핑에 @Column 을 읽으므로 jakarta.persistence-api 가 함께 있어야 한다.
    // 0.10.1 부터 엔티티 경로(insert·updateById·updateByMap)의 null 은 "미설정"이다. 컬럼을 NULL 로 비우는 것은 맵 경로뿐이다.
    implementation(libs.mybatis.repository)
    implementation(libs.jakarta.persistence.api)
    // JPA 는 쓰지 않는다. P6spyConfig 가 SQL 로그 정렬에 org.hibernate.engine.jdbc.internal.FormatStyle 을 쓰기 때문에 둔다.
